g.binread <-
function(binfile,start=0,end=0) {
int8 = function(x) {
   x[x < -128] = -128
   x[x > 127] = 127
   round(x)
...
## Read file header and return as a struct. Fail if version string is not present
	...
	if (all(idstr=="GENEA")) {
		...
		ends = c(0,find(header_items_raw==10))
		...
		hlen = length(ends)
 		for (i in 1:(hlen-1)) {
			...
			if (!isempty(hnames[i])) hvalues[i] = substr(hitem,separ+1,nchar(hitem))
        	}
...
find_timest = function(fid,noff,l,u,t) {
	...
	while (done == 0) {
		...
		while (done == 0) {
			...
			page = reshape(as.matrix(page[(1:(6*nchunks)),]),6,nchunks)
			## Unpack timestamps
...
doread = function(fid,pagestart,pageend,accel_gains,accel_offsets) {
	...
	while (!done) {
		...
		pages = reshape(as.matrix(pages),pagesize,npages)
		## Sort into 6-byte chunk, dropping trailing padding bytes
		pages = reshape(as.matrix(pages[1:(6*nchunks),]),6,nchunks*npages)
		## Find chunk types (which identify data vs. timestamp etc.)
		chunktypes = bitShiftR(pages[2,],4)
		...
		rawxyz[xyzidx,1] = pages[3,ct0] + bitShiftL(bitAnd(pages[2,ct0],15),8)
		rawxyz[xyzidx,2] = bitShiftL(pages[4,ct0],4) + bitShiftR(pages[5,ct0],4)
		rawxyz[xyzidx,3] = bitShiftL(bitAnd(pages[5,ct0],15),8) + pages[6,ct0]
...
getchunktypes = function(pages) {
	getchunktypes = bitShiftR(pages[2,],4)
}
...
unpack_timestamps = function(pages,chunktypes) {
	ct2 = (chunktypes==2)
	s24 = bitShiftL(uint32(pages[3,ct2]),24)
	s16 = bitShiftL(uint32(pages[4,ct2]),16)
	s8 = bitShiftL(uint32(pages[5,ct2]),8)
...
convert_xyz = function(rawxyz,accel_gains,accel_offsets,accidx,n_acc_chunks) {
	...
	rawxyz[xyzidx,] = rawxyz[xyzidx,] + repmat(accel_offsets,n_acc_chunks,1)
	rawxyz[xyzidx,] = int16((int32(rawxyz[xyzidx,]) * repmat(accel_gains,n_acc_chunks,1))/8192)
	## Convert to milliG
...
#	library(MASS)
#	library(signal)
#	durexp = length(G[,2])	#duration of experiment
...
#	VV1 = c(lb)	# lower boundary of the bandpass filter
#	VV2 = c(hb) 	# higher boundary of the bandpass filter
#	VV3 = c(n) 	# filter order
#	#--------------------------------------
#	# Band-pass filtering of signal with the "Signal"-package
#	# Buttord is a function that requires Ws[1] < Wp[1] < Wp[2] < Ws[2] for a pass band
#	Wc = matrix(0,2,1)
#	Wc[1,1] = VV1 / (fs/2)
#	Wc[2,1] = VV2 / (fs/2)
#	n = VV3
...
#	rm(Gy)
#	rm(Gz)
#	MICalc = sqrt((Gxfil[,1]^2) + (Gyfil[,1]^2) + (Gzfil[,1]^2))
#	MICalc
#}
#==========================================================================================
...
#G  = MICalc(rawxyz,lb=0.2,hb=18,fs,n=4)
t = timest
vt = vbat_t
ts = (t-t[1])
t = ts / 3600
