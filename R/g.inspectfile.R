...
function(datafile) {
  ...
  # Value:
  # header = fileheader
  # dformn = data format name (bin or csv)
  # dformc = data format code (1 = binary, 2 = csv)
  # monn = monitor name (genea, geneactive or Actigraph)
        ...
        if (ppp[2] != "NA:NA") {
          ...
          if (length(csvr) > 1) {
            for (ii in 1:nrow(csvr)) {
              tmp3 = unlist(strsplit(as.character(csvr[ii,1]),"quen:")) #part of 'frequency'
              if (length(tmp3) > 1) {
                #			print(csvr[ii,1])
