g.metric <-
  function(Gx,Gy,Gz,n,fs,ii,TW,lb,hb,mon) {
    ...
    if (ii == 1) {
      # high pass filter
      bf = signal::butter(n,c(lb/(fs/2)),type=c("high")) #creating filter coefficients
      ...
      bf = signal::butter(n,c(lb/(fs/2)),type=c("low")) #creating filter coefficients
      ...
      bf = signal::butter(n,c(hb/(fs/2)),type=c("low")) #creating filter coefficients
      Gxfil[,1] = filter(bf,Gx)	#filtering
      Gyfil[,1] = filter(bf,Gy)	#filtering
      Gzfil[,1] = filter(bf,Gz)	#filtering
      # vector magnitude minus one
