...
A critical step in going from the high-resolution data to the classification of activity types or energy expenditure is to calculate metrics, also known as signal features.
...
The table above indicates the approach used to separate the gravitation component from the acceleration signal.
...
However, they more likely represent the harmonics of low frequency movements and are thus part of the description of movement.
...
5\.
Studies who have criticised ENMO consistently failed to apply auto-calibration, or attempted to apply auto-calibration in a lab setting, ignoring the fact that auto-calibration is not designed for short lab settings.
...
Some GGIR users may like to use metrics not covered by GGIR.
To facilitate this, we allow for external function embedding as discussed in the vignette [Embedding external functions in GGIR](https://cran.r-project.org/package=GGIR/vignettes/ExternalFunction.html).
...
We are aware that this chapter could be more informative if it had an even more detailed discussion of each of the metrics.
However, given that our focus is on GGIR, we have compiled a list of related articles you may find useful:
